...
function check_unsupported_ie()
{
        ...
        return ((retv >= 0 && retv <= 8.0)?true:false);
}
...
function ApplyNas()
{
	...
	else if(service == 'samba')
	{
		...
		F2.samba_network_trash.value = (F.samba_network_trash.checked == true)?1:0;
        	if(document.getElementById('samba_allow_smb'))
			F2.samba_allow_smb.value = F.samba_allow_smb.value;
	}
	else if(service == 'url')
	{
	...
	else if(service == 'iptimecloud')	
	{
		if (GetRadioValue(F.iptimecloud_run) == 0)
		{
			F2.run.value="0";
        		F2.submit();
			MaskIt(document,'apply_mask');
		...
		if (F.iptimecloud_port.value == '')
		{
                        alert(NATCONF_INTAPPS_FTP_PORT_EMPTY);
                        F.iptimecloud_port.focus();
                        F.iptimecloud_port.select();
                        return;
		}
        
		if (checkRange(F.iptimecloud_port.value, 1, 65535) || (parseInt(F.iptimecloud_port.value) == 80))
		{
                        alert(NATCONF_INTAPPS_FTP_PORT_INVALID);
                        F.iptimecloud_port.focus();
                        F.iptimecloud_port.select();
		...
		if (GetValue(F.iptimecloud_path) == 'select_backup_folder')
		{
		        alert(MSG_SELECT_FOLDER_ERR);
                        F.iptimecloud_path.focus();
                        return;
		}

		F.iptimecloud_hddname.value = F.iptimecloud_path.value;
		F.iptimecloud_folder.value = "Home." + F.iptimecloud_userid.value;

		F2.run.value = GetRadioValue(F.iptimecloud_run);
		F2.port.value = F.iptimecloud_port.value;
		F2.port_method.value = (F.iptimecloud_port_method.checked == true)?"default":"";
		F2.hddname.value = F.iptimecloud_hddname.value;
		F2.folder.value = F.iptimecloud_folder.value;
	}
	...
	else if(service == 'entware')
	{
		if(!CheckEntware())	return;

		F2.run.value = GetRadioValue(F.entware_run);
		F2.entware_hddname.value = F.entware_hddname.value;
		F2.entware_folder.value = F.entware_folder.value;
		F2.entware_fname.value = F.entware_fname.value;
		F.entware_org_path.value = F.entware_hddname.value + ':' + F.entware_folder.value;
...
function InitSamba()
{
	...
	if(GetRadioValue(F.samba_run) == 0)
	{
		...
		DisableObj(F.samba_network_trash);
        	if(document.getElementById('samba_allow_smb'))
		{
			DisableObj(F.samba_allow_smb);
			F.samba_allow_smb.style.backgroundColor = "#eee";
		}
	...
	else
	{
		...
		EnableObj(F.samba_network_trash);
        	if(document.getElementById('samba_allow_smb'))
		{
			EnableObj(F.samba_allow_smb);
			F.samba_allow_smb.style.backgroundColor = "#fff";
		}
...
function InstallEntware()
{
        var F=document.nasmisc_fm;
        var F2=nas_service_iframe.main_form;

	if(!CheckEntware())	return;
	F2.service.value = 'entware';
	F2.install_entware.value = 'install';
	F2.entware_hddname.value = F.entware_hddname.value;
	F2.entware_folder.value = F.entware_folder.value;
	F.entware_org_path.value = F.entware_hddname.value + ':' + F.entware_folder.value;
	F2.submit();
        MaskIt(document,'apply_mask');
	ShowItDoc(document,'entware_dummyline');
	HideItDoc(document,'entware_radioline');
	HideItDoc(document, 'apply_bt');
	HideItDoc(document, 'install_entware_bt');
}

function CheckEntware()
{
        var F = document.nasmisc_fm;

	if(F.entware_default_admin && F.entware_default_admin.value == '1'){
		alert(NASCONF_ENTWARE_ACCESS_DENY);
		return false;
	}

	if (F.entware_new_folder.style.display != 'none' && (F.entware_new_folder.value =='' || F.entware_new_folder.value == F.entware_default_folder.value))
	{
		alert(MSG_NEW_FOLDER_ERR);
		F.entware_new_folder.focus();
		return false;
	}

	if (GetValue(F.entware_path) == 'select_entware_folder')
	{
		alert(MSG_SELECT_ENTWARE_FOLDER_ERR);
		F.entware_path.focus();
		return false;
	}

	if(GetValue(F.entware_path).match('_create_DxLoE2LK'))
	{
		var arr = GetValue(F.entware_path).split('_');
		var option_val, found;

		F.entware_hddname.value = arr[0];
		F.entware_folder.value = F.entware_new_folder.value;
		F.entware_fname.value = '';

		ret=CheckFolderStr(F.entware_new_folder.value,0);
		if(ret)
		{
			alert(ret);
			F.entware_new_folder.focus();
		...
		option_val = F.entware_hddname.value+':'+F.entware_folder.value;

		found = 0;
		for(i = 0 ; i < F.entware_path.length; i++)
		{
			if(F.entware_path[i].value == option_val)
			{
				F.entware_path.value = option_val;
				found = 1;
				break;
			}
		}
		...
		if(found == 0)
		{
			F.entware_path[F.entware_path.length] = new Option('/'+F.entware_hddname.value+'/'+F.entware_folder.value, option_val);
			F.entware_path.value = F.entware_hddname.value+':'+F.entware_folder.value;
		}

		ChangeEntwareFolder();
	}
	else if(GetValue(F.entware_path).match('select_clear_entware_folder'))
	{
		F.entware_hddname.value = '';
		F.entware_folder.value = '';
		F.entware_fname.value = 'clear';
	}
	else
	{
		var arr = GetValue(F.entware_path).split(':');

		F.entware_hddname.value = arr[0];
		F.entware_folder.value = arr[1];
		F.entware_fname.value = '';
	}
...
function UpdateEntwareStatus()
{
	var doc = document;
	if(!doc.nasmisc_fm)	doc = parent.document;
	var F = doc.nasmisc_fm;
	var ifr = doc.nas_service_iframe || doc.getElementsByName('nas_service_iframe')[0];
	if(!ifr)	return;
	var idoc = ifr.document || ifr.contentWindow.document;
	if(!idoc)	return;
	var F2 = idoc.main_form;
	var stat = F.entware_hidden_status.value;
	var stat_msg = '';

	if(F2 && F2.click_id && F2.click_id.value != 'entware')	return;

	EnableObj(F.entware_path);
	switch(stat){
		case 'INSTALLING':
			DisableObj(F.entware_path);
			doc.getElementById('entware_inline_status').innerHTML = NASCONF_ENTWARE_STATUS_INSTALLING;
			break;
		case 'ERROR':
			doc.getElementById('entware_inline_status').innerHTML = NASCONF_ENTWARE_STATUS_ERROR;
			break;
		case 'NONE':
			doc.getElementById('entware_inline_status').innerHTML = '-';
			break;
		case 'READY':
			doc.getElementById('entware_inline_status').innerHTML = NASCONF_ENTWARE_STATUS_READY;
			break;
		default:
			break;
	}
	...
	if(GetValue(F.entware_path) == 'select_entware_folder'){
		doc.getElementById('entware_inline_status').innerHTML = '-';
		ShowItDoc(doc,'entware_dummyline');
		HideItDoc(doc,'entware_radioline');
		HideItDoc(doc, 'apply_bt');
		HideItDoc(doc, 'install_entware_bt');
	}
	else if(GetValue(F.entware_path) == 'select_clear_entware_folder'){
		doc.getElementById('entware_inline_status').innerHTML = '-';
		ShowItDoc(doc,'entware_dummyline');
		HideItDoc(doc,'entware_radioline');
		ShowItDoc(doc, 'apply_bt');
		HideItDoc(doc, 'install_entware_bt');
	}
	else{
		switch(stat){
			case 'READY':
				if(GetValue(F.entware_path) == F.entware_org_path.value){
					HideItDoc(doc,'entware_dummyline');
					ShowItDoc(doc,'entware_radioline');
					ShowItDoc(doc, 'apply_bt');
					HideItDoc(doc, 'install_entware_bt');
				}else{
					ShowItDoc(doc,'entware_dummyline');
					HideItDoc(doc,'entware_radioline');
					HideItDoc(doc, 'apply_bt');
					ShowItDoc(doc, 'install_entware_bt');
					doc.getElementById('entware_inline_status').innerHTML = '-';
				}
				break;
			case 'INSTALLING':
...
function InitEntware()
{
	ChangeEntwareFolder();
}

function ChangeEntwareFolder()
{
        var F = document.nasmisc_fm;

	if(GetValue(F.entware_path).match('_create_DxLoE2LK'))
	{
		ShowObj(F.entware_new_folder);

           	F.entware_new_folder.value = F.entware_default_folder.value;
                F.entware_new_folder.style.color = "#aaaaaa";
	}
	else
		HideObj(F.entware_new_folder);
...
function ClickIptimeCloudPortMethod(default_port)
{
        var F = document.nasmisc_fm;

	if(!F.iptimecloud_port_method)
		return;
        if (F.iptimecloud_port_method.checked == false)
	{
		ReadOnlyObj(F.iptimecloud_port,0);
	}
	else
	{
		F.iptimecloud_port.value=default_port;
		ReadOnlyObj(F.iptimecloud_port,1);
	}

}


function InitIptimeCloud(default_port)
{
        var F = document.nasmisc_fm;
	if(GetRadioValue(F.iptimecloud_run) == 0)
	{
		if(F.iptimecloud_port_method) DisableObj(F.iptimecloud_port_method);
		DisableObj(F.iptimecloud_port);
		DisableObj(F.iptimecloud_userid);
		DisableObj(F.iptimecloud_passwd);
		DisableObj(F.iptimecloud_passwd_text);
		DisableObj(F.iptimecloud_password_view);
		DisableObj(F.iptimecloud_path);
		DisableObj(F.new_folder);
	}
	else
	{
		if(F.iptimecloud_port_method) EnableObj(F.iptimecloud_port_method);
		EnableObj(F.iptimecloud_port);
		EnableObj(F.iptimecloud_userid);
		EnableObj(F.iptimecloud_passwd);
		EnableObj(F.iptimecloud_passwd_text);
		EnableObj(F.iptimecloud_password_view);
		EnableObj(F.iptimecloud_path);
		EnableObj(F.new_folder);
	}
	ClickIptimeCloudPortMethod(default_port);
}


function ChangeIptimeCloudFolder()
{
        var F = document.nasmisc_fm;

	if(GetValue(F.iptimecloud_path).match('_create_DxLoE2LK'))
	{
		ShowObj(F.new_folder);
