...
function loadMenuHTML()
{
	...
	function bindEvents() {
        ...
        function loadHeader() {
                $.get( "/common/html/header.html" + '?' + Date.now(), function( responseTxt, statusTxt, xhr ) {
        ...
        function loadBody() {
                $.get( "html/main.html" + '?' + Date.now(), function( responseTxt, statusTxt, xhr ) {
			if( statusTxt === "success" )
				$body = $( responseTxt );
			_loadComplete();
		});
        ...
        function loadFooter() {
                $.get( "/common/html/footer.html" + '?' + Date.now(), function( responseTxt, statusTxt, xhr ) {
			if( statusTxt === "success" )
				$footer = $( responseTxt );
			_loadComplete();
		});
	...
	function loadMisc()
	{
                $.get( "/common/html/misc.html" + '?' + Date.now(), function( responseTxt, statusTxt, xhr ) {
			if( statusTxt === "success" )
				$misc = $( responseTxt );
			_loadComplete();
		});
	...
	function loadMenu()
	{
                $.get( "/common/html/menu.html" + '?' + Date.now(), function( responseTxt, statusTxt, xhr ) {
			if( statusTxt === "success" )
				$menu = $( responseTxt );
			_loadComplete();
		});
	...
	function deleteInvalidMenu( data ) {
		...
		for( var key in data )
			$("li[sid=" +  key + "]").val( "1" );
		var menuList = $("#menulist li").filter( function( index) {
			return $(this).val() != "1";
		});
		$( menuList ).remove();
	...
	function load_icon_clickevent()
	{
		...
		$('[sid="NETINFO_DHCPD"]').click(function(){location.href="/netinfo/dhcpd/iux.cgi";});
		...
		$('[sid="WIRELESSCONF_ADVANCESETUP"]').click(function(){location.href="/wirelessconf/advancesetup/iux.cgi";});
                $('[sid="WIRELESSCONF_EASYMESH"]').click(function(){location.href="/wirelessconf/easymesh/iux.cgi";});
		$('[sid="WIRELESSCONF_MACAUTH"]').click(function(){location.href="/wirelessconf/macauth/iux.cgi";});
                $('[sid="WIRELESSCONF_MESHTOOL"]').click(function(){location.href="/easymesh/";});
		...
		$('[sid="EXPERTCONF_VPN"]').click(function(){location.href="/expertconf/vpn/iux.cgi";});
		...
		$('[sid="EXPERTCONF_GAMINGVPN"]').click(function(){location.href="/expertconf/gamingvpn/iux.cgi";});
...
function iux_init(_tmenu, _smenu, _args, _retime, _use_local_status)
{
	...
	loadMenuHTML();
	page_backgroundColor(_tmenu, _smenu);
}
function page_backgroundColor(_tmenu, _smenu)
{
	var page = $(".ui-page");

        if(_tmenu==='expertconf' && _smenu==='advertise'){
                page.css("background-color", "#EEF1E6");
	}else if((_tmenu==='natrouterconf' && _smenu==='misc') || (_tmenu==='wirelessconf' && _smenu==='advancesetup')){
		page.css("background-color", "#F9FAF5");
	}else{
                page.css("background-color", "#F8F8F5");
        }	
...
function default_setup()
{
	...
	$(".ui-page-header-fixed").css("padding-top","6.5em");
//	$("li>a:even").css("background-color","#FFFFFF");
//	$("li>a:odd").css("background-color","#FFFFFF");//F9FAF5
...
function iux_submit(service_name , localpostdata, flag, configOption, formid, retime) 
{
	if(window.result_submit)
		$.ajaxSetup({async : true, timeout : 50000});	
	else
		$.ajaxSetup({ async : false });
...
{
	//add* 0123
	$('.content_field').children(':last').css('width','');
...
function confirm_close(flag)
{
	...
	//add* 0130
	$(".content_field").children(":eq(1)").css('width','');
